...
package object json4s
  ...
  private def asJsonOption404[A: Manifest]: ResponseAs[Either[ConsulResponseError, Option[A]], Any] =
    asStringAlways.mapWithMetadata { (str, meta) =>
      if (meta.isSuccess) {
        Try(deserializeJson[Option[A]].apply(str)).fold(
          err => Left(DeserializationException(str, new Exception(err))),
          res => Right(res)
        )
  ...
  private def asJsonOption404Extended[A: Manifest]: ResponseAs[Either[ConsulResponseError, Option[A]], Any] =
    asStringAlways.mapWithMetadata { (str, meta) =>
  ...
  private def asJson200or409[A: Manifest]: ResponseAs[Either[ConsulResponseError, A], Any] =
    asStringAlways.mapWithMetadata { (str, meta) =>
