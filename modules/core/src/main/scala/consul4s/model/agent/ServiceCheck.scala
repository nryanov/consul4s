 ...
 * @param Timeout - Specifies the frequency at which to run this check. This is required for HTTP and TCP checks.
 * @param Interval - Specifies a timeout for outgoing connections in the case of a Script, HTTP, TCP, or gRPC check. Can be specified in the form of "10s" or "5m" (i.e., 10 seconds or 5 minutes, respectively).
 ...
 * @param Interval - Specifies a different HTTP method to be used for an HTTP check. When no value is specified, GET is used.
 * @param Timeout - Specifies a set of headers that should be set for HTTP checks. Each header can have multiple values.
 * @param Header - Specifies a body that should be sent with HTTP checks.
 * @param Method - Specifies the frequency at which to run this check. This is required for HTTP and TCP checks.
 * @param Body - - Specifies a timeout for outgoing connections in the case of a Script, HTTP, TCP, or gRPC check. Can be specified in the form of "10s" or "5m" (i.e., 10 seconds or 5 minutes, respectively).
 * @param CheckID - Specifies a unique ID for this check on the node. This defaults to the "Name" parameter, but it may be necessary to provide an ID for uniqueness.
 * @param ServiceID - Specifies the ID of a service to associate the registered check with an existing service provided by the agent.
 * @param Status - Specifies the initial status of the health check.
 * @param Notes - Specifies arbitrary information for humans. This is not used by Consul internally.
