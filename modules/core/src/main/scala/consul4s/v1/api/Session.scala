...
trait Session[F[_]] { this: ConsulApi[F] =>
  ...
  /**
   * PUT /session/create This endpoint initializes a new session. Sessions must be associated with a node and may be associated with any
   * number of checks.
   * @param newSession
   *   - session definition
   * @param dc
  ...
  /**
   * PUT /session/destroy/:uuid This endpoint destroys the session with the given name. If the session UUID is malformed, an error is
   * returned. If the session UUID does not exist or already expired, a 200 is still returned (the operation is idempotent).
   * @param sessionId
   *   - session ID
   * @param dc
   *   - Specifies the datacenter to query. This will default to the datacenter of the agent being queried. This is specified as part of the
  ...
  /**
   * PUT /session/renew/:uuid This endpoint renews the given session. This is used with sessions that have a TTL, and it extends the
