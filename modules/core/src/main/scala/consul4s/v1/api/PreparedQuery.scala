...
trait PreparedQuery[F[_]] { this: ConsulApi[F] =>
   ...
   * @param queryUUID
   *   - Specifies the UUID of the query to execute. This is required and is specified as part of the URL path. This can also be the name of
   *   an existing prepared query, or a name that matches a prefix name for a prepared query template.
   * @param dc
   ...
   * @param connect
   *   - If true, limit results to nodes that are Connect-capable only. This can also be specified directly on the template itself to force
   *   all executions of a query to be Connect-only. See the template documentation for more information.
   * @param token
   *   - consul token
   * @param cacheMode
   *   - see [[CacheMode]]
