...
object ConsistencyMode extends StringEnum[ConsistencyMode] {
   ...
   * The trade-off is fast reads but potentially stale values. The condition resulting in stale reads is hard to trigger,
   * and most clients should not need to worry about this case. Also, note that this race condition only applies to reads,
   * not writes.
   */
  case object Default extends ConsistencyMode("default")

  /**
   * This mode is strongly consistent without caveats. It requires that a leader verify with a quorum of peers that
   * it is still leader. This introduces an additional round-trip to all server nodes.
